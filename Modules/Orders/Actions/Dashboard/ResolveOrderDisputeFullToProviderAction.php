...
class ResolveOrderDisputeFullToProviderAction
{
    public function __construct(
        private readonly OrderRepositoryInterface $orderRepository,
        private readonly LogOrderStatusHistory $logStatusHistory,
        private readonly SettleOrderPaymentAction $settleOrderPaymentAction,
    ) {}
    ...
    public function handle(
        ...
        return DB::transaction(function () use ($order, $admin, $notes) {
            ...
            $resolution = OrderDisputeResolutionEnum::FullProvider;

            $order = Order::withoutEvents(
                fn () => $this->orderRepository->update($order, [
                    'status' => OrderStatusEnum::EndedViaDispute,
                ])
            );

            $this->settleOrderPaymentAction->handle($order->fresh());

            $this->logStatusHistory->handle(
                $order,
                $admin,
                $fromStatus,
                OrderStatusEnum::EndedViaDispute->value,
