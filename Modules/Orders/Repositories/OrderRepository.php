...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function listWindowedForProviderHome(Provider $provider): Collection
    {
        ...
        return $this->providerHomeScopedOrdersQuery($provider)
            ->with([
                ...
                'offers' => fn ($query) => $query->where('provider_id', $provider->id),
            ])
            ->withCount(['offers', 'media'])
    ...
    public function dashboardStats(): array
    {
        ...
        return LookupCache::rememberFor('stats:orders:dashboard', 30, fn (): array => [
            ...
            'disputed' => Order::where('status', OrderStatusEnum::Disputed)->count(),
            'completed' => Order::whereIn('status', OrderStatusEnum::endedAggregateValues())->count(),
            'cancelled' => Order::whereIn('status', OrderStatusEnum::cancelledAggregateValues())->count(),
