...
The `opportunities:expire` command runs hourly via the scheduler. It dispatches `ExpireOpportunityJob` for each opportunity past its `expires_at` date with status `new` or `offer_accepted`.
