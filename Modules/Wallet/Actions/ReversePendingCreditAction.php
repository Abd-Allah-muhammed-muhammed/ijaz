...
class ReversePendingCreditAction
{
    ...
    public function handle(Model $owner, float $amount, Model $operation, string $description = ''): void
    {
        ...
        $wallet->decrement('pending_credit', $amount);

        $this->transactionRepo->create($wallet, $owner, new WalletTransactionData(
            amount: $amount,
            description: $description ?: 'Reverse pending credit for '.$operation::class.'#'.$operation->getKey(),
