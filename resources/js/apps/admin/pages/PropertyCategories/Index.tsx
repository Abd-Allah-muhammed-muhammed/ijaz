...
const Index = ({ rows, prams }: Props) => {
  ...
  const searchParamsChanged = (name: keyof SearchParams, value: string | number) => {
    ...
    visitWithFilters(PropertyCategoryController.index().url, next, { only: ['rows', 'prams'] });
  };
      ...
      <Content>
        <KTCard className="p-6">
          <DataTable
            ...
            onRowClick={(row) => {
              router.visit(PropertyCategoryController.edit(row.id as number).url);
            }}
            toolbar={
            ...
            actions={() => [
              {
                id: 'edit',
                label: t('edit'),
                href: (row) => PropertyCategoryController.edit(row.id as number).url,
              },
              {
                id: 'delete',
                label: t('delete'),
