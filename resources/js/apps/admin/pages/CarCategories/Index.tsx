...
const Index = ({ rows, prams }: Props) => {
  ...
  const searchParamsChanged = (name: keyof SearchParams, value: string | number) => {
    ...
    visitWithFilters(CarCategoryController.index().url, next, { only: ['rows', 'prams'] });
  };
      ...
      <Content>
        <KTCard className="p-6">
          <DataTable
            ...
            onRowClick={(row) => {
              router.visit(CarCategoryController.edit(row.id as number).url);
            }}
            toolbar={
            ...
            actions={() => [
              {
                id: 'edit',
                label: t('edit'),
                href: (row) => CarCategoryController.edit(row.id as number).url,
              },
              {
                id: 'delete',
                label: t('delete'),
