...
const HeaderNotificationsMenu: FC = () => {
  ...
  const refreshUnreadCount = useCallback(async () => {
    try {
      const {data} = await apiClient.get<SingleApiResponse<UnreadCountPayload>>(
        NotificationController.unreadCount.url(),
      )
    ...
    try {
      const [{data: listResponse}, {data: countResponse}] = await Promise.all([
        apiClient.get<SingleApiResponse<NotificationListPayload>>(
          NotificationController.index.url({query: {per_page: 20}}),
        ),
  ...
  useEffect(() => {
    ...
    window.addEventListener(PROVIDER_NOTIFICATION_RECEIVED_EVENT, onReceived)
    return () => {
      window.removeEventListener(PROVIDER_NOTIFICATION_RECEIVED_EVENT, onReceived)
    }
  ...
  const markAsRead = async (notification: ProviderNotification) => {
    ...
    try {
      await apiClient.post(NotificationController.markAsRead.url(notification.id))
      setItems((prev) =>
        prev.map((item) =>
  ...
  const markAllAsRead = async () => {
    ...
    try {
      await apiClient.post(NotificationController.markAllAsRead.url())
      setItems((prev) =>
