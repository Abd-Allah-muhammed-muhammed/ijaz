...
const Index = (
  ...
  const searchPramsChanged = (name: keyof SearchPrams, value: string | number) => {
    ...
    router.get(CategoryController.index().url, searchPrams);
  };
  return (
    <>
          ...
          <Table
            ...
            headers={[
              ...
              },
              {
                title: t('fees'),
                property: 'fees',
                render: row => `${row.fees_type.value === CategoryFeesTypeEnum.INHERITED ? row.fees_type.label : (row.fees || 0)}`,
              },
              {
                title: t('fees_type'),
                property: 'fees_type',
                render: row => `${row.fees_type.label}`,
              }
            ]}
            actions={[
              {
                ...
                ele: (row) => (
                  <LinkAction
                    ...
                    href={CategoryController.edit(row.id as number).url}
                    title={t('edit')}
                  />
                ),
              },
