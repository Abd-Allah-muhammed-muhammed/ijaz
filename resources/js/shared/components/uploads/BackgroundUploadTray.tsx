...
export type BackgroundUploadTrayProps = {
  ...
  /** Prefix for data-pan attributes (e.g. registration-upload-tray). */
  panPrefix: string;
};

/**
 * Collapsed-by-default fixed tray showing background upload progress.
 * Shared by provider registration and profile required-files upload.
 */
export default function BackgroundUploadTray({
  entries,
  onRetry,
  panPrefix,
          ...
          <ul className="list-unstyled mb-0 px-3 py-2 border-top">
            {list.map((entry) => (
              <li
                ...
                data-pan={`${panPrefix}-row-${entry.id}`}
              >
                <div className="min-w-0">
                  <div className="fw-semibold text-gray-800 text-truncate">
                    {entry.fileName}
                  </div>
                ...
                <div className="flex-shrink-0">
                  {entry.status === 'done' ? (
                    <span className="text-success fw-bold" aria-label="done">
                      ✓
                    </span>
                  ) : null}
                  {entry.status === 'failed' ? (
