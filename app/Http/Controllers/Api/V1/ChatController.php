...
class ChatController extends Controller
{
     ...
     * @throws Throwable
     */
    public function sendToProvider(Request $request, Provider $provider)
    {
        DB::beginTransaction();
        try {
     ...
     * @throws Throwable
     */
    public function sendToSupport(SendSupportMessageRequest $request)
    {
        DB::beginTransaction();
        try {
            $user = auth()->user();
    ...
    public function guaranteeChatStore(Request $request, GuaranteeRequest $guaranteeRequest): JsonResponse
    {
        $chat = Conversation::firstOrCreate([
            'operation_type' => get_class($guaranteeRequest),
            'operation_id' => $guaranteeRequest->id,
        ], [
            'user1_id' => $guaranteeRequest->user->id,
            'user1_type' => get_class($guaranteeRequest->user),
            'user2_id' => $guaranteeRequest->provider->id,
            'user2_type' => get_class($guaranteeRequest->provider),

        ]);

        return $this->successResponse(
            ConversationResource::make($chat->load(['lastMassage.sender', 'lastMassage.lastAttachment', 'user2', 'user1']))
        );
    }

    public function guaranteeChatIndex(): JsonResponse
    {
        $user = auth()->user();
        $conversations = Conversation::latest('last_message_at')
            ->select('conversations.*')
            ->where('operation_type', GuaranteeRequest::class)
            ->join('guarantee_requests', function ($join) {
                $join
                    ->on('guarantee_requests.id', 'conversations.operation_id')
                    ->where('guarantee_requests.status', '!=', GuaranteeRequestStatusEnum::EndedByClient);
            })
