...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    | opcodesio/log-viewer has no built-in redact option. These patterns are
    | applied by App\Support\LogRedactor when rendering log entries (via
    | RedactingLaravelLog) and when writing logs (via RedactSensitiveDataProcessor).
    |
    */

    'redact' => [
        'enabled' => env('LOG_VIEWER_REDACT', true),

        'patterns' => [
            [
                'pattern' => '/(?i)(\*{0,2}\s*authorization\s*\*{0,2}\s*[:=]\s*)(Bearer\s+)?\S+/',
            ...
            [
                'pattern' => '/(?i)(\*{0,2}\s*cookie\s*\*{0,2}\s*[:=]\s*)[^\r\n]+/',
                'replace' => '$1[REDACTED]',
            ],
            [
                'pattern' => '/(?i)\b(XSRF-TOKEN|ijaz_session|laravel_session|remember_web[_a-z0-9]*)=[^;\s\"\'&]+/',
                'replace' => '$1=[REDACTED]',
            ],
            [
                'pattern' => '/(?i)((?:password|passwd|pwd)\s*[\"\']?\s*[:=]\s*[\"\']?)[^\"\'\s,&}]+([\"\']?)/',
                'replace' => '$1[REDACTED]$2',
            ],
            [
                'pattern' => '/(?i)((?:api[_-]?key|access[_-]?token|refresh[_-]?token|client_secret|secret)\s*[\"\']?\s*[:=]\s*[\"\']?)[^\"\'\s,&}]+([\"\']?)/',
                'replace' => '$1[REDACTED]$2',
            ],
        ],
    ],
