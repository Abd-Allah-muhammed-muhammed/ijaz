...
| **Chat** | Member / order / ticket conversations, realtime events, chat handlers. **Attachments:** Spatie MediaLibrary on `ConversationMessage` (collection `attachments`, default disk `public`). Image attachments get a queued `webp` conversion via shared `HasWebpImageConversion` on the dedicated `media-conversions` queue (original preserved); API prefers the WebP URL once generated, otherwise falls back to the original. **Requires an active queue worker listening to `media-conversions`** — without one, originals still display. Legacy `conversation_attachments` table/model fully retired. |
...
- **`resources/js/Enums/*` is gitignored.** Same pattern: `make:js-enums` is HMR-only unless run in `prebuild`. A stale `Chat.ts` without `ChatEventEnum.Typing` makes Echo listen for literal `.undefined` instead of `.typing`. `prebuild` / deploy must run `php artisan make:js-enums`.
...
| Chat attachments | MediaLibrary on `ConversationMessage` (`attachments` collection, default disk `public` — S3 later is a disk config change). Image attachments: queued `webp` via `HasWebpImageConversion` on queue **`media-conversions`** (keep original; resource uses WebP URL when ready, else original). **Queue worker must listen to `media-conversions`** (appended on default Supervisor/Horizon supervisor); stalled queue = graceful degradation to original. PDFs skipped. Legacy `conversation_attachments` **retired**. |
| MediaLibrary WebP (Phase 2) | Shared trait `App\Support\HasWebpImageConversion`. **Converted (images only):** Chat attachments, Classifieds advisement media, Orders media, Opportunity `files`, Jobs media, GuarantorRequest `files` (images only). **Excluded (integrity / KYC):** Guarantor `signature`, GuarantorCompanyDetail collections (`authorized_id`, `contracts`, `iban_certificates`, `company_documents`), Provider verification docs on `local` disk (`id_image`, commercial record, IBAN/freelancer certs). Config: `config/media-library.php` → `queue_name` = `media-conversions` (`MEDIA_QUEUE`). |
| PHP upload limits | **Laravel `max:5120` (5MB) is the real UX-facing limit.** PHP `upload_max_filesize` / `post_max_size` are only a generous abuse safety net (recommend **64M**); they must never be the primary control. If PHP still rejects a body, `PostTooLargeException` → MMAE validation JSON envelope in `bootstrap/app.php`. See README → Server Setup. |
...
- Chat attachments: Spatie MediaLibrary on `ConversationMessage` only. Legacy `conversation_attachments` table/model fully retired. Image → queued WebP conversion on `media-conversions` (original kept); API falls back to original until conversion completes — keep a queue worker listening to `media-conversions` (default Supervisor list / Horizon `supervisor-default`). Same trait covers Classifieds / Orders / Opportunity / Jobs / Guarantor `files` images; Guarantor signatures + company KYC + Provider verification docs on `local` are **not** converted.
