...
| **Chat** | Member / order / ticket conversations, realtime events, chat handlers. **Attachments:** Spatie MediaLibrary on `ConversationMessage` (collection `attachments`, default disk `public`). Image attachments get a queued `webp` conversion (original preserved); API prefers the WebP URL once generated, otherwise falls back to the original. **Requires an active queue worker** for WebP generation — without one, originals still display. Legacy `conversation_attachments` table/model fully retired. |
...
| Chat attachments | MediaLibrary on `ConversationMessage` (`attachments` collection, default disk `public` — S3 later is a disk config change). Image attachments: queued `webp` conversion (keep original; resource uses WebP URL when ready, else original). **Queue worker required** for conversion; stalled queue = graceful degradation to original. PDFs skipped. Legacy `conversation_attachments` **retired**. |
...
- Chat attachments: Spatie MediaLibrary on `ConversationMessage` only. Legacy `conversation_attachments` table/model fully retired. Image → queued WebP conversion (original kept); API falls back to original until conversion completes — keep a queue worker running in every environment that accepts chat uploads.
